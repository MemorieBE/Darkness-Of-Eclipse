...
{
    [Header("Audio Source")]
    public AudioSource[] ambienceSoundFirst = new AudioSource[1]; //!< The ambience sound firsts.
    public AudioSource[] ambienceSoundSecond = new AudioSource[1]; //!< The ambience sound seconds.
    public float[] isolatedVolume = new float[1]; //!< The set volume for each audio source.
    ...
    public float fadeTime = 1f; //!< The fading time in seconds.
    public float autoFader = 2f; //!< Multiplied by the set fade time to determine the minumum length of the audio clip in seconds before using an automatic fading setting.
    public float fadeOffset = 0f; //!< The amount of seconds to cut off the end of the audioclip.
